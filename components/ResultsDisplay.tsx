...
import type { CalculationResult, IngredientSet } from '../types';
    ...
    <ul className="space-y-2">
      {Object.entries(ingredients)
        ...
        <li key={key} className={`flex justify-between items-center p-3 rounded-lg ${index % 2 === 0 ? 'bg-slate-100 dark:bg-slate-800/60' : 'bg-transparent'}`}>
          ...
          <span className="font-semibold text-lg text-gray-900 dark:text-gray-100">{(value as number).toFixed(1)}g</span>
        </li>
      ))}
      {ingredients.total && ingredients.total > 0 && (
         <li className="flex justify-between items-center p-3 bg-slate-200 dark:bg-slate-700 rounded-lg mt-3">
          ...
          <span className="font-bold text-xl text-gray-900 dark:text-gray-100">{ingredients.total.toFixed(0)}g</span>
        </li>
      )}
    </ul>
  </div>
    ...
    const formatIngredients = (ingredients: IngredientSet) => {
      return Object.entries(ingredients)
        ...
        .map(([key, value]) => {
          ...
          return `${formattedKey}: ${(value as number).toFixed(2)}g`;
        })
        .join('\n');
    };

    let message = `🍕 *My Pizza Dough Recipe* 🍕\n\n`;
    message += `*Total Flour:* ${results.totalFlour.toFixed(2)}g\n`;
    message += `*Total Dough:* ${results.totalDoughWeight.toFixed(2)}g\n\n`;

    if (results.preferment) {
      message += `*${results.preferment.name}*\n`;
      ...
      <Card>
        <div className="space-y-6">
          <div className="text-center">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm bg-slate-100 dark:bg-slate-900/50 p-4 rounded-lg">
              <div>
                  ...
                  <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{results.totalFlour.toFixed(0)}g</p>
              </div>
              <div>
                  <p className="font-semibold text-slate-600 dark:text-slate-400">Total Dough</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{results.totalDoughWeight.toFixed(0)}g</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 pt-6 border-t border-slate-200 dark:border-slate-700">
            <button
              onClick={handleShare}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-white bg-[#25D366] rounded-lg hover:bg-[#1DA851] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#25D366] dark:focus:ring-offset-slate-800 transition-colors duration-200 shadow-lg shadow-green-500/10 hover:shadow-green-500/20"
            >
