    ...
    <ul className="space-y-2">
      {Object.entries(ingredients)
        ...
        .map(([key, value], index) => {
          ...
          let displayKey: string = t(`ingredients.${key}`);
          ...
          return (
            <li key={key} className={`flex justify-between items-center p-3 rounded-lg ${index % 2 === 0 ? 'bg-slate-100 dark:bg-slate-800/60' : 'bg-transparent'}`}>
              <span className="capitalize font-medium text-gray-700 dark:text-gray-300">{displayKey}</span>
      ...
      {ingredients.total && ingredients.total > 0 && (
         <li className="flex justify-between items-center p-3 bg-slate-200 dark:bg-slate-700 rounded-lg mt-3">
          <span className="font-bold text-gray-800 dark:text-gray-200">{t('results.total')}</span>
      ...
      <div className="sticky top-8">
        <Card>
          <div className="text-center text-gray-500 py-12">
            ...
            <h3 className="mt-2 text-lg font-semibold">{t('results.empty.title')}</h3>
            <p className="mt-1 text-sm text-slate-500">{t('results.empty.subtitle')}</p>
          </div>
        </Card>
      </div>
    );
  ...
  const handleShare = () => {
    ...
    const formatIngredients = (ingredients: IngredientSet) => {
      return Object.entries(ingredients)
        .filter(([key, value]) => typeof value === 'number' && value > 0 && key !== 'name' && key !== 'total')
        .map(([key, value]) => {
          return `${t(`ingredients.${key}`)}: ${(value as number).toFixed(1)}gr`;
        })
        .join('\n');
    };

    let message = `🍔 *${t('share.title', { style: t(`doughStyles.${params.doughStyle}`) })}* 🍔\n\n`;

    message += `*${t('share.config.title')}:*\n`;
    message += `- ${t('share.config.method')}: ${t(`doughMethods.${params.doughMethod}`)}\n`;
    message += `- ${t('share.config.balls')}: ${params.ballCount} x ${params.ballWeight}gr\n`;

    if (params.doughStyle === 'Buns') {
      message += `- ${t('ingredients.milk')}: ${params.hydration}%\n`;
      message += `- ${t('ingredients.salt')}: ${params.salt}%\n`;
      if (params.oliveOil > 0) message += `- ${t('ingredients.butter')}: ${params.oliveOil}%\n`;
      if (params.sugar > 0) message += `- ${t('ingredients.sugar')}: ${params.sugar}%\n`;
      if (params.wholeEgg > 0) message += `- ${t('ingredients.wholeEgg')}: ${params.wholeEgg}\n`;
      if (params.eggYolk > 0) message += `- ${t('ingredients.eggYolk')}: ${params.eggYolk}\n`;
      if (params.malt > 0) message += `- ${t('ingredients.malt')}: ${params.malt}%\n`;
    } else {
      message += `- ${t('ingredients.hydration')}: ${params.hydration}%\n`;
      message += `- ${t('ingredients.salt')}: ${params.salt}%\n`;
      if (params.oliveOil > 0) message += `- ${t('ingredients.oliveOil')}: ${params.oliveOil}%\n`;
      if (params.malt > 0) message += `- ${t('ingredients.malt')}: ${params.malt}%\n`;
    }
    ...
    if (params.doughMethod === 'Biga') {
      message += `*${t('share.biga.title')}:*\n`;
      message += `- ${t('share.biga.percentage')}: ${params.bigaPercentage}%\n`;
      message += `- ${t('share.biga.hydration')}: ${params.bigaHydration}%\n`;
      message += `- ${t('share.biga.yeast')}: ${params.bigaFreshYeast}%\n\n`;
    }

    if (params.doughMethod === 'Poolish') {
      message += `*${t('share.poolish.title')}:*\n`;
      message += `- ${t('share.poolish.percentage')}: ${params.poolishPercentage}%\n`;
      message += `- ${t('share.poolish.hours')}: ${params.poolishHours} ${t('units.hours')}\n\n`;
    }
    ...
    if (params.doughStyle === 'Teglia Romana' || params.doughStyle === 'Focaccia') {
      ...
      message += `*${t('share.tray.title')}:*\n`;
      if (shape === 'round') {
        const diameter = isTeglia ? params.tegliaDiameter : params.focacciaDiameter;
        message += `- ${t('share.tray.shape')}: ${t('tegliaShapes.round')} (${diameter}cm)\n`;
      } else {
        ...
        message += `- ${t('share.tray.shape')}: ${t('tegliaShapes.square')} (${length} x ${width}cm)\n`;
      }
      message += `- ${t('share.tray.thickness')}: ${t(`tegliaThicknesses.${thickness}`)}\n\n`;
    }

    message += `*${t('share.totals.title')}:*\n`;
    message += `- ${t('results.totalFlour')}: ${results.totalFlour.toFixed(0)}gr\n`;
    message += `- ${t('results.totalDough')}: ${results.totalDoughWeight.toFixed(0)}gr\n\n`;

    if (results.preferment) {
      message += `*${t('share.ingredientsTitle', { context: results.preferment.name })}:*\n`;
      message += `${formatIngredients(results.preferment)}\n\n`;
    }

    message += `*${t('share.ingredientsTitle', { context: t('results.finalDough') })}:*\n`;
        ...
        <div className="space-y-6">
          <div className="text-center">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm bg-slate-100 dark:bg-slate-900/50 p-4 rounded-lg">
              <div>
                  <p className="font-semibold text-slate-600 dark:text-slate-400">{t('results.totalFlour')}</p>
                  <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{results.totalFlour.toFixed(0)}gr</p>
              </div>
              <div>
                  <p className="font-semibold text-slate-600 dark:text-slate-400">{t('results.totalDough')}</p>
          ...
          <hr className="border-slate-200 dark:border-slate-700" />
          <IngredientTable title={t('results.finalDough')} ingredients={results.finalDough} params={params} />
